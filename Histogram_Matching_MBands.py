...
def load_mat_file(file_string):
    ...
    # Perform histogram matching on the three mbands.
    hm1 = histogram_matching(mband1,mband2)
    hm2 = histogram_matching(hm1,mband3)

    # Calculate and plot the m band histograms.
    hist1,bins1 = np.histogram(mband1.flatten(),256,normed=True)
    hist2,bins2 = np.histogram(mband2.flatten(),256,normed=True)
    hist3,bins3 = np.histogram(mband3.flatten(),256,normed=True)
    ...
    plt.plot(hist3, '-y')
    plt.plot(hist4, '--k')
    plt.legend(('mband1','mband2','mband3','hist match'), loc = 'upper right')
    plt.title('M Band Histograms')

    # Calculate and plot the m band cdf. 
    cdf1 = hist1.cumsum()
    ...
    plt.legend(('mband1','mband2','mband3','hist match'), loc = 'lower right')
    plt.title('M Band cdf')

    # Display the m band histogram matching result.
    plt.subplot(233)
    plt.imshow(hm2)
    plt.title('M Band Histogram Matching')
    plt.colorbar()
